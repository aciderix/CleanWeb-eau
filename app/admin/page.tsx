      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-gray-800 rounded-2xl p-8 w-full max-w-md shadow-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 mt-2">Clean&apos;Eau Nantes — Gestion des événements</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 flex justify-between items-center">
        <h1 className="text-xl font-bold">🌊 Clean&apos;Eau Admin — Événements</h1>
        <div className="flex gap-3">
          <button
            onClick={() => { setEditingEvent({ ...emptyEvent, sort_order: events.length + 1 }); setIsNew(true); }}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        event.is_visible ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
                      }`}>
                        {event.is_visible ? 'Visible' : 'Masqué'}
                      </span>
                      <span className="text-xs text-gray-500">Ordre: {event.sort_order}</span>
                    </div>
                    <p className="text-gray-400 text-sm mb-1">📅 {event.date} — 📍 {event.location}</p>
                    <p className="text-gray-300 text-sm">{event.description}</p>
                    {event.link && (
                      <p className="text-cyan-400 text-sm mt-1">🔗 {event.link_text}: {event.link}</p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditingEvent({ ...editingEvent, sort_order: parseInt(e.target.value) })}
                      className="w-full bg-gray-700 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                    />
                  </div>
                  <div className="flex items-end pb-1">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setEditingEvent({ ...editingEvent, is_visible: e.target.checked })}
